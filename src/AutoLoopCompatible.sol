...
abstract contract AutoLoopCompatible is AutoLoopCompatibleInterface, AccessControlEnumerable {
    ...
    function safeTransferAdmin(address newAdminAddress) public {
        ...
        require(newAdminAddress != address(0), "cannot transfer admin to zero address");
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Only current admin can transfer their role"
        );
    ...
    function acceptTransferAdminRequest() public {
        require(_msgSender() == adminTransferRequest, "Only new admin can accept transfer request");
        require(adminTransferRequestOrigin != address(0), "No pending transfer request to accept.");
