...
contract CrumbleCoreSnap is CrumbleCore {
    constructor(uint256 a, uint256 b, uint256 c, uint16 d, uint16 e, uint256 f, uint256 g)
...
contract OracleRunSnap is OracleRun {
    constructor(uint256 a, uint256 b, uint256 c, uint256 d, uint32 e, uint32 f, uint32 g)
...
contract GameGasSnapshotTest is Test {
    ...
    function test_Gas_CrumbleCore_StandardTick() public {
        CrumbleCoreSnap game =
            new CrumbleCoreSnap(0.01 ether, 0.002 ether, 60, 10_000, 100, 1000, 5000);
        vm.prank(alice);
    ...
    function test_Gas_GladiatorArena_BoutResolution() public {
        GladiatorArenaSnap game =
            new GladiatorArenaSnap(0.01 ether, 0.001 ether, 60, 500, 500, 50, 8);
        for (uint256 i = 0; i < 4; i++) {
            address p = vm.addr(0x3000 + i);
            vm.deal(p, 1 ether);
            vm.prank(p);
            uint256 id = game.mintGladiator{value: 0.01 ether}();
            vm.prank(p);
            game.enterBout{value: 0.001 ether}(id);
        }
        vm.warp(block.timestamp + 60);
        uint256 g = gasleft();
    ...
    function test_Gas_MechBrawl_BrawlResolution() public {
        MechBrawlSnap game = new MechBrawlSnap(0.01 ether, 0.001 ether, 60, 500, 500, 50, 8);
        for (uint256 i = 0; i < 4; i++) {
            address p = vm.addr(0x5000 + i);
            vm.deal(p, 1 ether);
            vm.prank(p);
            uint256 id = game.deployMech{value: 0.01 ether}();
            vm.prank(p);
            game.joinBrawl{value: 0.001 ether}(id);
        }
        vm.warp(block.timestamp + 60);
        uint256 g = gasleft();
        game.tickForTest(bytes32(uint256(100)));
    ...
    function test_Gas_SorcererDuel_DuelResolution() public {
        SorcererDuelSnap game = new SorcererDuelSnap(0.01 ether, 0.001 ether, 60, 500, 500, 50, 8);
        for (uint256 i = 0; i < 4; i++) {
            address p = vm.addr(0x6000 + i);
            vm.deal(p, 1 ether);
            vm.prank(p);
            uint256 id = game.summonSorcerer{value: 0.01 ether}();
            vm.prank(p);
            game.enterDuel{value: 0.001 ether}(id);
        }
        vm.warp(block.timestamp + 60);
        uint256 g = gasleft();
        game.tickForTest(bytes32(uint256(100)));
    ...
    function test_Gas_KaijuLeague_ClashResolution() public {
        KaijuLeagueSnap game = new KaijuLeagueSnap(0.01 ether, 0.001 ether, 60, 500, 500, 50, 8);
        for (uint256 i = 0; i < 4; i++) {
            address p = vm.addr(0x7000 + i);
            vm.deal(p, 1 ether);
            vm.prank(p);
            uint256 id = game.hatchKaiju{value: 0.01 ether}();
            vm.prank(p);
            game.enterClash{value: 0.001 ether}(id);
        }
        vm.warp(block.timestamp + 60);
        uint256 g = gasleft();
        game.tickForTest(bytes32(uint256(100)));
    ...
    function test_Gas_VoidHarvester_MissionResolution() public {
        VoidHarvesterSnap game = new VoidHarvesterSnap(0.01 ether, 0.001 ether, 60, 500, 500, 50, 8);
        for (uint256 i = 0; i < 4; i++) {
            address p = vm.addr(0x8000 + i);
            vm.deal(p, 1 ether);
            vm.prank(p);
            uint256 id = game.deployProbe{value: 0.01 ether}();
            vm.prank(p);
            game.launchMission{value: 0.001 ether}(id);
        }
        vm.warp(block.timestamp + 60);
        uint256 g = gasleft();
        game.tickForTest(bytes32(uint256(100)));
    ...
    function test_Gas_GladiatorOracle_Settlement() public {
        ArenaSnapForOracle snap =
            new ArenaSnapForOracle(0.01 ether, 0.001 ether, 60, 500, 500, 50, 8);
        GladiatorOracle game = new GladiatorOracle(address(snap), 120, 120, 0.001 ether, 500);
        ...
        vm.deal(p2, 1 ether);
        vm.prank(p1); // g1
        snap.mintGladiator{value: 0.01 ether}();
        vm.prank(p2); // g2
        snap.mintGladiator{value: 0.01 ether}();
        vm.prank(p1);
        snap.enterBout{value: 0.001 ether}(1);
        vm.prank(p2);
        snap.enterBout{value: 0.001 ether}(2);
    ...
    function test_Gas_OracleRun_Expedition() public {
        OracleRunSnap game = new OracleRunSnap(0.01 ether, 0.002 ether, 60, 500, 300, 50, 400);
        for (uint256 i = 0; i < 4; i++) {
    ...
    function test_Gas_OracleRun_Wipe() public {
        OracleRunSnap game = new OracleRunSnap(0.01 ether, 0.002 ether, 60, 500, 900, 50, 100);
        for (uint256 i = 0; i < 4; i++) {
    ...
    function test_Gas_KaijuOracle_Settlement() public {
        LeagueSnapForKaijuOracle snap =
            new LeagueSnapForKaijuOracle(0.01 ether, 0.001 ether, 60, 500, 500, 50, 8);
        KaijuOracle game = new KaijuOracle(address(snap), 120, 120, 0.001 ether, 500);
        ...
        address p1 = vm.addr(0xF101);
        address p2 = vm.addr(0xF102);
        vm.deal(p1, 1 ether);
        vm.deal(p2, 1 ether);
        vm.prank(p1); // k1
        snap.hatchKaiju{value: 0.01 ether}();
        vm.prank(p2); // k2
        snap.hatchKaiju{value: 0.01 ether}();
        vm.prank(p1);
        snap.enterClash{value: 0.001 ether}(1);
        vm.prank(p2);
        snap.enterClash{value: 0.001 ether}(2);

        KaijuOracle.Round memory r = game.getRound(1);
        vm.warp(r.commitEndAt);
        for (uint256 i = 0; i < 3; i++) {
            vm.prank(players[i]);
            game.reveal(kids[i], salts[i]);
        }

        uint256 t =
            r.revealEndAt > snap.lastClashAt() + 60 ? r.revealEndAt : snap.lastClashAt() + 60;
        vm.warp(t);
    ...
    function test_Gas_ForecasterLeaderboard_Distribution() public {
        LeagueSnapForKaijuOracle snap =
            new LeagueSnapForKaijuOracle(0.01 ether, 0.001 ether, 60, 500, 500, 50, 8);
        KaijuOracle oracleGame = new KaijuOracle(address(snap), 60, 60, 0.001 ether, 500);
        ForecasterLeaderboard board =
            new ForecasterLeaderboard(address(oracleGame), 3600, 3, 20, 500);
        ...
        for (uint256 i = 0; i < 3; i++) {
            bytes32 c = keccak256(abi.encode(kids[i], salts[i], players[i]));
            vm.prank(players[i]);
            oracleGame.commit{value: 0.001 ether}(c);
        }

        address lp1 = vm.addr(0xF201);
        address lp2 = vm.addr(0xF202);
        vm.deal(lp1, 1 ether);
        vm.deal(lp2, 1 ether);
        vm.prank(lp1);
        snap.hatchKaiju{value: 0.01 ether}();
        ...
        for (uint256 i = 0; i < 3; i++) {
            vm.prank(players[i]);
            oracleGame.reveal(kids[i], salts[i]);
        }
