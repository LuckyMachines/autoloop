...
contract ForecasterLeaderboardTest is Test {
    ...
    function setUp() public {
        ...
        league = new LeagueForLeaderboard(
            KAIJU_FEE, ENTRY_FEE, CLASH_INTERVAL, PROTOCOL_RAKE, 500, 50, 8
        );
        ...
        leaderboard = new ForecasterLeaderboard(
            address(oracle), DISTRIBUTION_INTERVAL, TOP_N, MAX_ROUNDS_PER_TICK, PROTOCOL_RAKE
        );

        // Pre-mint two permanent kaijus owned by k1Owner and k2Owner
        vm.prank(k1Owner); // kaijuId = 1
        league.hatchKaiju{value: KAIJU_FEE}();
        vm.prank(k2Owner); // kaijuId = 2
        league.hatchKaiju{value: KAIJU_FEE}();
    }
    ...
    function test_ConstructorRejectsZeroOracle() public {
        ...
        new ForecasterLeaderboard(
            address(0), DISTRIBUTION_INTERVAL, TOP_N, MAX_ROUNDS_PER_TICK, PROTOCOL_RAKE
        );
    }
    ...
    function test_ConstructorRejectsZeroTopN() public {
        ...
        new ForecasterLeaderboard(
            address(oracle), DISTRIBUTION_INTERVAL, 0, MAX_ROUNDS_PER_TICK, PROTOCOL_RAKE
        );
    }
    ...
    function test_ConstructorRejectsTopNTooHigh() public {
        vm.expectRevert("ForecasterLeaderboard: topN range");
        new ForecasterLeaderboard(
            address(oracle), DISTRIBUTION_INTERVAL, 11, MAX_ROUNDS_PER_TICK, PROTOCOL_RAKE
        );
    }
    ...
    function test_ConstructorRejectsHighRake() public {
        ...
        new ForecasterLeaderboard(
            address(oracle), DISTRIBUTION_INTERVAL, TOP_N, MAX_ROUNDS_PER_TICK, 2001
        );
    }
    ...
    function test_MultipleForecasterScores() public {
        ...
        vm.warp(r.commitEndAt + 1);
        vm.prank(alice);
        oracle.reveal(1, bytes32(uint256(1)));
        vm.prank(bob);
        oracle.reveal(2, bytes32(uint256(2)));
        vm.prank(carol);
        oracle.reveal(1, bytes32(uint256(3)));
    ...
    function test_TiedForecastersSharePrize() public {
        ...
        oracle.reveal(1, bytes32(uint256(1)));
        vm.prank(bob);
        oracle.reveal(1, bytes32(uint256(2)));
